...
ISURIDE には、利用者が使用するクライアントアプリと、椅子のオーナーが使用するオーナーアプリケーションの二種類があります。
また、今回 ISUCON 競技者の動作デバッグ確認用のシミュレーターが準備されています。

### クライアントアプリケーションの動作確認方法

ISURIDE には、サーバーの Web ブラウザから HTTPS でアクセスできます。

#### (1) シミュレーター (/simulator)

`isuride.xiv.isucon.net`にアクセスし、`Simulator Application`を押すことで遷移できます。
アカウントプリセットに記載のチェアを利用しアプリケーションをシミュレートする機能を提供しています。
何点か注意事項があるので、ご注意ください。
...
- ②チェア設定画面: 
  - 上からチェアの名前,椅子のモデル,配車依頼の状態の表示と、チェア位置ボタン,進捗インジケーターがあります。
  -チェア位置ボタンを押すことで椅子の位置を設定できます。
  - 進捗インジケーターは、配車依頼の進捗をクライアント側で計算して表示しています。黒いピンが`PICKUP `状態を表し、赤いピンが`ARRIVED`状態を表します。
- ③配車を受け付けるボタン: 
  チェアのactivityを切り替えることができます。
- ④擬似チェア表示機能: 
  クライアントアプリケーション上に架空の椅子のデータを表示し、現実で稼働した状態の表示に近づけるための機能です。そのため、表示されるチェアはデータベース上にないものも表示されます。

##### 注意事項
シミュレーターで操作できるチェアは、空車(`MATCHING `状態か`COMPLETED `状態)のみになります。起動中のシミュレーターのクライアント画面でISURIDEボタンを押し、配車依頼を出すと、1秒置きにチェアが移動し、配車完了(`COMPLETED `状態)までシュミレートできます。

配車依頼が起動中のシミュレーター画面以外で出された場合、` 現在、他のシミュレーターが使用中です`と表示され操作不可能になりますが、一定時間経過後に自動的に配車完了までシミュレートされます。


#### (2) クライアント (/client)

`isuride.xiv.isucon.net`にアクセスし、`Client Application`を押すことで遷移できます。
フォームに値を入力し登録を行うか、ログインボタンを遷移しpresetからユーザーを選択することでアプリケーションを起動できます。

現在地と目的地を選択することで、配車依頼を出すことができます。

#### (3) オーナー (/owner)

`isuride.xiv.isucon.net`にアクセスし、`Owner Application`を押すことで遷移できます。
フォームに値を入力し登録を行うか、ログインボタンを遷移しpresetからユーザーを選択することでアプリケーションを起動できます。

椅子タブで、オーナーが所有するチェアの情報と、売上タブで椅子別,モデル別の売り上げ情報を見ることができます
