...
const main = async() => {
  ...
  while(true) {
  await sleep(1000)
  const fetched = await fetch("http://localhost:8080/api/chair/notification", {
    "method": "GET",
    "headers": {
      "Cookie":  `chair_session=${targetChair.token}`,
    },
  ...
  if (status === "MATCHING") {
    ...
    while(true) {
      ...
      await fetch("http://localhost:8080/api/chair/coordinate", {
        "method": "POST",
        "body": JSON.stringify({
          latitude: currentCoordinate.latitude + (pickup_coordinate.latitude - currentCoordinate.latitude)/2,
          longitude: currentCoordinate.longitude + (pickup_coordinate.longitude - currentCoordinate.longitude)/2,
        }),
        "headers": {
          "Cookie":  `chair_session=${targetChair.token}`,
        },
    ...
    if (status === "PICKUP") {
      ...
      while(true) {
        ...
        await fetch("http://localhost:8080/api/chair/coordinate", {
          "method": "POST",
          "body": JSON.stringify({
            latitude: currentCoordinate.latitude + (destination_coordinate.latitude - currentCoordinate.latitude)/2,
            longitude: currentCoordinate.longitude + (destination_coordinate.longitude - currentCoordinate.longitude)/2,
          }),
          "headers": {
            "Cookie":  `chair_session=${targetChair.token}`,
          },
