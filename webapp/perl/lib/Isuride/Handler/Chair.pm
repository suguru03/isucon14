...
sub chair_post_chairs ($c) {
    ...
    my $owner = $c->dbh->select_row('SELECT * FROM owners WHERE chair_register_token = ?', $params->{chair_register_token});
...
sub chair_post_coordinate ($c) {
    ...
    my $location = $c->dbh->select_row('SELECT * FROM chair_locations WHERE id = ?', $chair_location_id);
    die unless $location;
    ...
    if (defined $ride) {
        ...
        if ($status ne 'COMPLETED' && $status ne 'CANCELED') {
            if ($params->{latitude} == $ride->{pickup_latitude} && $params->{longitude} == $ride->{pickup_longitude} && $status eq 'ENROUTE') {
                $c->dbh->query('INSERT INTO ride_statuses (id, ride_id, status) VALUES (?, ?, ?)', ulid(), $ride->{id}, 'PICKUP');
            }

            if ($params->{latitude} == $ride->{destination_latitude} && $params->{longitude} == $ride->{destination_longitude} && $status eq 'CARRYING') {
                $c->dbh->query('INSERT INTO ride_statuses (id, ride_id, status) VALUES (?, ?, ?)', ulid(), $ride->{id}, 'ARRIVED');
            }
        }
    }
    $txn->commit;
...
sub chair_get_notification ($c) {
    ...
    my $status;
    my $yet_sent_ride_status = $c->dbh->select_row('SELECT * FROM ride_statuses WHERE ride_id = ? AND chair_sent_at IS NULL ORDER BY created_at ASC LIMIT 1', $ride->{id});
    ...
    my $user = $c->dbh->select_row('SELECT * FROM users WHERE id = ? FOR SHARE', $ride->{user_id});
    ...
    if (defined $yet_sent_ride_status) {
        $c->dbh->query('UPDATE ride_statuses SET chair_sent_at = CURRENT_TIMESTAMP(6) WHERE id = ?', $yet_sent_ride_status->{id});
    }
...
sub chair_post_ride_status ($c) {
    ...
    my $ride = $c->dbh->select_row('SELECT * FROM rides WHERE id = ? FOR UPDATE', $ride_id);
    ...
    match($params->{status} : eq) {
        case ('ENROUTE') {
            $c->dbh->query('INSERT INTO ride_statuses (id, ride_id, status) VALUES (?, ?, ?)', ulid(), $ride_id, 'ENROUTE');
        }
        case ('CARRYING') {
            my $status = get_latest_ride_status($c, $ride->{id});
            ...
            $c->dbh->query('INSERT INTO ride_statuses (id, ride_id, status) VALUES (?, ?, ?)', ulid(), $ride_id, 'CARRYING');
        }
        default {
